...
call(['cp',inputCaseFile, inputCaseFileNew])


while(cluster_rank <= n_clusters_reported):
    
    cluster_loc_ids = []    
    res_tmp = open(outputDirPath + 'res_' + str(cluster_rank), 'w')    
    
    print "SaTScan iteration number: " + str(cluster_rank)
    # ./SaTScanBatch64 + inputParamFile
    call(["/home/vgc/flegros/satscan/software/SaTScanBatch64", inputParamFile])
    ...
    res_tmp.write(line.replace("1.Loc", str(cluster_rank)+".Loc"))    
    ...
    while(line[2:6]!="Test"):        
        ...
        if (line[0:len("  Time frame")] != "  Time frame"): # collect cluster timespan to update data
            res_tmp.write(line)
        else:        
            line = line.split(":")[1]
            line = line.replace(" ", "")
            ...
            res_tmp.write("  Time frame............: " + intervalToDatetime(cluster_startint)  + " to " + intervalToDatetime(cluster_endint) + "\n")
    ...
    # 2.) Updating clusters location values in raw data at /sit_xxk_yyh/Casgraph1110_d.cas
