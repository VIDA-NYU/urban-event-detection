...
public class TLCData {
	...
	// New function for grid processing is setupGrid()
	/*
	 ...
	 */
	public void setupGrid(double longMin, double longMax, double latMin, double latMax, double gridRes) {
		// Create grid cells and add center of grid cells as polygons
		try {
			
			double R_earth = 6371000;
			
			double gridWidth = 2*Math.PI*cos(latMin*Math.PI/180)*R_earth* (Math.abs(longMax-longMin)/360);
			double gridHeight = 2*Math.PI*R_earth* (Math.abs(latMax-latMin)/360);
			
			int nrows = int(gridHeight / gridRes);
			int ncols = int(gridWidth / gridRes);
			
			longMax = (ncols*gridRes*180 / (Math.PI*cos(latMin*Math.PI/180)*R_earth)) + longMin; //truncate the last partial grid cells on the borders of the bounded area
			latMax = (nrows*gridRes*180 / (Math.PI*R_earth)) + latMin;
			
			double latStep = Math.abs(latMax-latMin)/nrows;
			double longStep = Math.abs(longMax-longMin)/ncols;
			...
			for(int i = 0; i < nrows; i ++) {
				...
				for(int j = 0; j < ncols; j ++) {
					double x = longMin + (i+0.5)*longStep;
					double y = latMin + (j+0.5)*latStep;					
			...
			index = new GridIndex(4096, 4096); //##// 4096 ???
			index.buildGrid(polygons);
...
//		}
	...
	public void writeFunctions() {
		try {
			new File("pick-density").mkdir();
			// new File("drop-density").mkdir();
...
//				String dfol = "drop-";
				...
				PrintStream pden = new PrintStream(pfol + "density/density" + ts);
				// PrintStream dden = new PrintStream(dfol + "density/density" + ts);
